...
@app.route('/')
def index():
    # Load main visualization (current behavior, from 'usuário_da_ferramenta.json')
    main_example_path = os.path.join('static', 'json', 'usuário_da_ferramenta.json')
    nodes_data_main = []
    edges_data_main = []
    if os.path.exists(main_example_path):
        try:
            with open(main_example_path, 'r', encoding='utf-8') as f:
                json_data_main = json.load(f)
            nodes_main, edges_main = transformar_para_vis(json_data_main)
    ...
    # Load static examples for the dropdown (current behavior)
    examples_dir_static = os.path.join('static', 'json', 'examples')
    if not os.path.exists(examples_dir_static):
        os.makedirs(examples_dir_static)
    example_file_paths_static = glob.glob(os.path.join(examples_dir_static, '*.json'))
    example_filenames_static = sorted([os.path.basename(p) for p in example_file_paths_static])

    # Fetch all analyses from the database
    try:
        saved_analyses = get_all_analyses() # This returns a list of Row objects
    except Exception as e:
        app.logger.error(f"Error fetching analyses from database: {e}")
        saved_analyses = []

    return render_template('index.html',
                           nodes_data=nodes_data_main, # For the main visualization
                           edges_data=edges_data_main, # For the main visualization
                           example_files=example_filenames_static, # For the static examples dropdown
                           saved_analyses=saved_analyses) # New: list of analyses from DB
...
def analise_route(): # Renamed to avoid conflict with imported 'analisar'
    ...
    if texto_entrada:
        analysis_name = texto_entrada[:50] + "..." if len(texto_entrada) > 50 else texto_entrada
...
def view_analysis_route(analysis_uuid):
    ...
    try:
        ...
        narrative_text = analysis_record['narrative_text']
